...
# output
output_fasta_file="./tmp/filtered.fasta"
output_file="./tmp/filtered_pred.tsv"

awk -F'\t' '{print "AFDB:AF-"$1"-F1"}' $filtered_file | seqkit grep -f - $fasta_file > $output_fasta_file
