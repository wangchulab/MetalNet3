...
# tmp
domain_info_file="./tmp/repId-hasCATH-isNovel-isSymm.tsv"
...
wc -l $domain_info_file # 1295371
awk '{sum += $2; next} END {print sum}' $domain_info_file # 1162203
awk '{sum += $3; next} END {print sum}' $domain_info_file # 3
awk '{sum += $4; next} END {print sum}' $domain_info_file # 5